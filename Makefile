...
IMAGE_BUILD_ARGS = --build-arg BUILD_DATE=$(BUILD_DATE) --build-arg BUILD_VERSION=$(BASE_VERSION)
GCLOUD_EXTRA_FLAGS =
...
build-nbanow-image: build-nbanow-matchfunction-image build-nbanow-director-image build-nbanow-evaluator-image build-telemetry-image build-frontend-image build-backend-image build-query-image build-synchronizer-image build-base-build-image

build-telemetry-image: build-grafana-image build-prometheus-image build-base-build-image

build-grafana-image: build-base-build-image
	docker build -f install/grafana/Dockerfile -t $(REGISTRY)/openmatch-grafana:$(TAG) install/grafana

build-prometheus-image: build-base-build-image
	docker build -f install/prometheus/Dockerfile -t $(REGISTRY)/openmatch-prometheus:$(TAG) install/prometheus
...
## deploy
deploy-core:
	(cd install/; docker stack deploy -c docker-compose.core.yml $(NAMESPACE))
...
#
#deploy-large:
#	(cd install/; docker stack deploy -c docker-compose.core.yml -c docker-compose.demo.yml -c docker-compose.telemetry.yml $(NAMESPACE))
#
#deploy-scale:
#	(cd install/; docker stack deploy -c docker-compose.core.yml -c docker-compose.scale.yml -c docker-compose.telemetry.yml $(NAMESPACE))
...
build/chart/index.yaml: build/toolchain/bin/helm$(EXE_EXTENSION) build/chart/open-match-$(BASE_VERSION).tgz
...
pkg/pb/%.pb.go: api/%.proto third_party/ build/toolchain/bin/protoc$(EXE_EXTENSION) build/toolchain/bin/protoc-gen-go$(EXE_EXTENSION) build/toolchain/bin/protoc-gen-grpc-gateway$(EXE_EXTENSION)
	...
	mv $(REPOSITORY_ROOT)/build/prototmp/ns-open-match/$@ $@
...
build/cmd/%/BUILD_PHONY:
	...
	CGO_ENABLED=0 $(GO) build -a -installsuffix cgo -o $(BUILD_DIR)/cmd/$*/run ns-open-match/cmd/$*
...
ci-deploy-artifacts: install/yaml/ $(SWAGGER_JSON_DOCS) build/chart/
ifeq ($(_GCB_POST_SUBMIT),1)
...
.PHONY: docker update-deps sync-deps all build proxy-dashboard proxy-prometheus proxy-grafana clean clean-build clean-toolchain clean-binaries clean-protos presubmit test ci-reap-namespaces md-test vet
